...
# of the scanning area for the scanner within it, written as depth: range. Each layer has a thickness of exactly 1. A layer at depth 0 begins immediately inside the firewall; a layer at depth 1 would start immediately after that.
#
...
# your packet enters it, you are caught. (If a scanner moves into the top of its layer while you are there, you are
# not caught: it doesn't have time to notice you before you leave.) If you were to do this in the configuration above,
...
def move_into(layers, scanners, pos):
    if scanners.get(pos) == 0:
        return pos * layers[pos]
    else:
        return None


def scan_cycle(layers, scanners):
    # Scanners move up and down, emulate this with modulo (depth*2)-2
    return {k: (v + 1) % ((layers[k] * 2) - 2) for k, v in scanners.items()}
    ...
    for pos in range(0, max_layer + 1):
        ...
        if new_cost is not None:
            if cost is None:
                ...
                cost += new_cost
        scanners = scan_cycle(layers, scanners)

    return cost


def go_undetected(layers):
    max_layer = max(layers.keys())
    start = scanners = {k: 0 for k, _ in layers.items()}
    delay = 0
    pos = 0

    while pos <= max_layer:
        if move_into(layers, scanners, pos) is not None:
            pos = 0
            # Backtrack to where we last started.
            start = scanners = scan_cycle(layers, start)
            delay += 1
        else:
            pos += 1
            scanners = scan_cycle(layers, scanners)
